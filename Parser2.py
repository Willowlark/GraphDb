...
# document = """Write a page that shows some data. I don't particularly care what it is, but there should be enough to be interesting. (At least 50 numbers. If you're only plotting one thing, such as "US population over time", then get 50 years' worth of data. If you're plotting returns of 10 stocks, do at least 5 days' worth, or something like that. If you choose something that seems good but there aren't 50 numbers ("maximum high jump at Olympic Games" would be stuck with only 29), let me know and if I like it I'll okay it.) Make a pretty graph of all your data. (Just one graph. Don't make 10 different pages to show 10 different stocks, draw them all on the same graph in 10 different colors.) If there's a lot of data make it respond to mouseovers. (See the Sunrise/Sunset grapher for an example; it's at http://elvis.rowan.edu/~kilroy/sunrise/sunrise.html . That gives 3 numbers for each of 365 days, for 1095 values. Printing that many numbers on the canvas would make it unreadable, so they got moved to mouseovers.)
...
def _new_get_continuous_chunks_NP(tagged):
    ...
    befores, afters = defaultdict(list), defaultdict(list)
    suffixes = []
    suffix_token = None
    token = None
    for chunked in nltk.ne_chunk_sents(tagged, binary=False):
        ...
        for i in chunked:
            if type(i) == nltk.Tree:
                ...
                befores[token].extend(suffixes)
                if current_chunk:
                    current_chunk = []
                    suffixes = []
                else:
                    pass
                suffixes.extend(token.split(" "))
                if suffix_token is not None:
                    afters[suffix_token].extend(token.split(" "))
                suffix_token = token
            else:
                suffixes.append(i[0])
                if token is not None:
                    afters[token].append(i[0])

    return dict(labels), dict(counts), dict(befores), dict(afters)


def _newer_get_continuous_chunks_NP(tagged, n):
    """
    Experimental
    """
    labels = defaultdict(set) # the dictionary of labels whose values are associated sets of topics
    counts =  defaultdict(int)  # the dictionary of topics whose value is the count of appearances in the entire body of text
    before, afters = defaultdict(list), defaultdict(list)
    for chunked in nltk.ne_chunk_sents(tagged, binary=False):
        current_chunk = []
        for x in range(0, len(chunked)):
            ...
            if type(i) == nltk.Tree:
                ...
                counts[token] += 1
                current_chunk.append(token)
                suffix_index = 0 if x-n < 0 else x-n
                prefix_index = len(chunked) if x+n > len(chunked) else x+n
                for elem in chunked[x+1:prefix_index]:
                    afters[token].append(" ".join([token for token, pos in elem.leaves()]) if type(elem) == nltk.Tree else elem[0])
                for elem in chunked[suffix_index:x]:
                    before[token].append(" ".join([token for token, pos in elem.leaves()]) if type(elem) == nltk.Tree else elem[0])
    return dict(labels), dict(counts), dict(before), dict(afters)


def main(n):
    processed = preproc(document)
    listing = []
    # labels, counts, before, after = _newer_get_continuous_chunks_NP(processed, n)
    labels, counts, before, after = _new_get_continuous_chunks_NP(processed)
    ...
    for elem in set(listing):
        print blue(repr(elem))
        print '\t', vars(elem)
        print " ".join(elem.before), green(repr(elem)), " ".join(elem.after)
        print
